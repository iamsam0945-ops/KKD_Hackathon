...
export default function JoinPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  function next(e: React.FormEvent) { e.preventDefault(); if (!form.name.trim()||!form.phone.trim()) return; setError(''); setStep(2) }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault(); if (!form.username.trim()||!form.password.trim()) return
    setLoading(true); setError('')
    try {
      const res = await fetch('/api/auth/register', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({...form,referralToken:token}) })
    ...
    <div className="min-h-screen bg-[#0d0824] flex flex-col items-center justify-center px-5">
      <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-black text-white" style={{textShadow:'0 0 16px rgba(167,139,250,0.7)'}}>You&apos;re Invited!</h1>
          <p className="text-violet-300 text-sm mt-1 font-semibold">Sign up & get a FREE scratch card</p>
        </div>
          ...
          <AnimatePresence mode="wait">
            {step===1 ? (
              <motion.form key="s1" initial={{opacity:0,x:-28}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-28}}
                ...
                <p className="text-violet-400 text-xs text-center font-bold">Step 1 of 2</p>
                {[{key:'name',label:'👤 Your Name',placeholder:'Priya Kapoor',type:'text',ac:'name'},{key:'phone',label:'📱 Phone Number',placeholder:'9876543210',type:'tel',ac:'off'}].map(f=>(
                  <div key={f.key}>
                    <label className="text-violet-300 text-xs font-black uppercase tracking-wider mb-2 block">{f.label}</label>
                    <input type={f.type} placeholder={f.placeholder} value={form[f.key as keyof typeof form]} onChange={e=>setForm(p=>({...p,[f.key]:e.target.value}))} autoComplete={f.ac} required
                ...
                <motion.button whileTap={{y:5,boxShadow:'0 1px 0 #3b0764'}} type="submit" className="btn-candy-violet w-full py-4 text-base mt-2">Continue →</motion.button>
              </motion.form>
            ) : (
              <motion.form key="s2" initial={{opacity:0,x:28}} animate={{opacity:1,x:0}} exit={{opacity:0,x:28}}
                ...
                <p className="text-violet-400 text-xs text-center font-bold">Step 2 of 2</p>
                <div>
                  <label className="text-violet-300 text-xs font-black uppercase tracking-wider mb-2 block">🎮 Username</label>
                  <input type="text" placeholder="priya_yoga" value={form.username} onChange={e=>setForm(f=>({...f,username:e.target.value}))} autoComplete="off" required
                    className="w-full bg-black/30 border-2 border-violet-500/40 rounded-2xl px-4 py-3 text-white placeholder-white/25 focus:outline-none focus:border-violet-400 text-sm font-semibold transition-all" />
                </div>
                <div>
                  <label className="text-violet-300 text-xs font-black uppercase tracking-wider mb-2 block">🔐 Password</label>
                  <div className="relative">
                    <input type={showPassword?'text':'password'} placeholder="••••••••" value={form.password} onChange={e=>setForm(f=>({...f,password:e.target.value}))} autoComplete="new-password" required
                      className="w-full bg-black/30 border-2 border-violet-500/40 rounded-2xl px-4 py-3 pr-11 text-white placeholder-white/25 focus:outline-none focus:border-violet-400 text-sm font-semibold transition-all" />
                    <button type="button" onClick={()=>setShowPassword(v=>!v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-violet-300 hover:text-white text-sm">{showPassword?'🙈':'👁️'}</button>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <motion.button whileTap={{y:5,boxShadow:'0 1px 0 #047857'}} type="submit" disabled={loading} className="btn-candy-green flex-1 py-3.5 text-base disabled:opacity-50">
